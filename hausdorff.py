...
def distance_to_polytope(x,P, metric=1):
    ...
    if metric == 2:
        Q = sp.linalg.block_diag(np.eye(len(x)),np.zeros((len(P_vertices),len(P_vertices))))
        ...
        sol = solve_qp(Q,q,A=A,b=b,lb=lb,ub=ub,solver='osqp')
        sol = sol[:len(x)] 
        ...
        lambda_coefs = np.concatenate([P_vertices.T,[np.ones(len(P_vertices))],[np.zeros(len(P_vertices))]],axis=0)
        mu_coefs = np.concatenate([B_vertices.T,[np.zeros(len(B_vertices))],[np.ones(len(B_vertices))]],axis=0)
        ...
        dist = prog.x[-1]
        sol = np.sum([prog.x[i]*P_vertices[i] for i in range(len(P_vertices))],axis=0) 
        return dist,sol
