...
from tropical_reconstruction.polytope import Polytope, Zonotope, random_polytope, random_zonotope
import os
...
if polytope:
    ...
    P = Polytope(pts=random_zonotope(5,2).vertices)
    warmstart = False

lrscheduler = MultiplicityLRScheduler(start=0.005,scale=1)
opt_kwargs = {"normalize_grad":True, "lrscheduler":lrscheduler}
Z = approximate_by_zonotope(P, GradientOptimizer, 600, 5, opt_kwargs=opt_kwargs, closeness_thresh=0.96, warmstart=warmstart)
