...
class TZLP_Solver:
    ...
    def _get_equations(self):
        ...
        for eps, y in zip(self.Epsilon, self.U):
            ...
            equations['LP'].append(Equation(rhs, "E", variables, lin=dict(zip(*lin_expr))))
        ...
        for i in range(self.M):
            for j in range(self.n):
                ...
                equations["SLP"].append(
                    Equation(0,"G",variables,lin=dict(zip(*lin_expr)),quad={(self.w_name(i), self.x_name(j)): quad_val})
                )
    ...
    def check_feasible(self,sol,verbose=True):
        ...
        substituted_equations['SLP'] = list(map(substitute,substituted_equations['SLP']))
        ...
        SLP_eqs = np.array([eq.numpy()[0] for eq in substituted_equations['SLP']],dtype=np.float64)
        SLP_rhs = np.array([eq.numpy()[1] for eq in substituted_equations['SLP']],dtype=np.float64)
        lp = linprog(c=np.zeros(SLP_eqs.shape[1]), A_ub=SLP_eqs, b_ub=SLP_rhs, bounds=(None, None))
        feasible_pt = lp.x
        if lp.status == 2:
            return False
        ...
        if self.quad is not None:
            for m, c in self.quad.items():
                ...
                if c > 0:
                    s += " + "
                if c != 0:
                    s += self._format_coef(abs(c), float_precision=float_precision) + "".join(m)
